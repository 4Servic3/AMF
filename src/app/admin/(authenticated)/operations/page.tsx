...
export default async function OperationsPage() {
  ...
  const supabase = await adminDb("ops.manage");
  ...
  queryError(webhookError);
  queryError(jobError);
  return (
    <div className="space-y-8 max-w-7xl mx-auto">
      <PageHeader
        title="Monitoramento"
        description="Webhooks recebidos e tarefas em segundo plano. Retornar à fila solicita nova tentativa; a execução depende do processador."
      />
            ...
            <tbody>
              {webhooks?.map((wh: any) => (
                <tr key={wh.id} className="border-b last:border-0">
                  ...
                  <td className="p-3">{wh.type}</td>
                  <td className="p-3">{wh.status}</td>
                  <td className="p-3 text-right">
                    {wh.status === "failed" ? (
                      <ActionForm
                        action={reprocessWebhook.bind(null, wh.id)}
                        label="Retornar à fila"
                      >
              ...
              {!webhooks?.length && (
                <tr>
                  <td colSpan={4} className="p-6 text-center text-amf-ink-400">
              ...
              {jobs?.map((job: any) => (
                <tr key={job.id} className="border-b last:border-0">
                  ...
                  <td className="p-3 text-right">
                    {job.status === "failed" ? (
                      <ActionForm
                        action={requeueJob.bind(null, job.id)}
                        label="Retornar à fila"
                      >
                        <span className="sr-only">
                          Solicitar nova tentativa
