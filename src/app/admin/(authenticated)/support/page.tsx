...
export default async function SupportPage() {
  ...
  const supabase = await adminDb("support.manage");
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto">
      <PageHeader
        title="Suporte"
        description="Tickets abertos e histórico de atendimento."
      />
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {tickets?.map((ticket: any) => (
              ...
              >
                <td className="p-4 font-mono text-xs text-amf-ink-400">
                  {ticket.id.slice(0, 8)}
                </td>
                <td className="p-4">{ticket.subject}</td>
                <td className="p-4">
                  <span
                    className={`px-2 py-0.5 rounded-full text-xs font-medium ${statusColor[ticket.status] || "bg-gray-100 text-gray-500"}`}
                  >
                    {ticket.status}
                  </span>
                </td>
                <td className="p-4 text-amf-ink-400">
                  {new Date(ticket.created_at).toLocaleDateString("pt-BR")}
                </td>
                <td className="p-4">
                  <Link
                    href={`/admin/support/${ticket.id}`}
                    className="text-amf-teal-600 hover:underline"
                  >
                    Ver
                  </Link>
                </td>
            ...
            {!tickets?.length && (
              <tr>
                <td colSpan={5} className="p-8 text-center text-amf-ink-400">
