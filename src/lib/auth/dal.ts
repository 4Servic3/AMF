...
export async function writeAdminAuditEvent({
  ...
  const validResourceId =
    resourceId && /^[0-9a-f-]{36}$/i.test(resourceId) ? resourceId : null;
    ...
    .insert({
      ...
      details:
        validResourceId || !resourceId
          ? details
          : { ...details, resourceKey: resourceId },
      reason,
    });
  if (auditError)
    console.error("Falha ao registrar auditoria", auditError.code);
}
