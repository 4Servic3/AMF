...
type PublishedRow = { id: string; title: string; case_story_videos: { id: string; caption: string | null; created_at: string; status: string }[] }

export async function getPublishedCaseStories(): Promise<PublishedCase[]> {
  ...
  return (data as PublishedRow[] || []).map(item => ({
    ...
    items: [...item.case_story_videos].sort((a, b) => a.created_at.localeCompare(b.created_at) || a.id.localeCompare(b.id))
      .map(story => ({ id: story.id, caption: story.caption || '' }))
  }))
}
