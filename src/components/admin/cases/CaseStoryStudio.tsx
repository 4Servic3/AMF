...
export default function CaseStoryStudio({ cases, stories, selectedCase = '' }: { cases: CaseOption[]; stories: CaseStoryRow[]; selectedCase?: string }) {
  ...
  async function publish() {
    if (!file || busy || opening || recording || publishing.current || (!caseId && !caseTitle.trim())) return
  ...
  return <section className="space-y-5 rounded-2xl border border-amf-border bg-white p-5">
    <div><h2 className="text-xl font-semibold">Publicar story de um caso</h2><p className="text-sm text-amf-muted">Grave ou envie um vídeo ou imagem, confira e escolha o caso. Após publicar, o story aparecerá na página inicial para todos os usuários conectados. Vídeos aparecem quando o processamento terminar.</p></div>
    <div className="flex flex-wrap gap-3">
      <button type="button" disabled={busy || opening} onClick={() => recording ? (recorder.current?.state === 'recording' && recorder.current.stop()) : void record()} className="rounded-full bg-amf-petrol-700 px-5 py-2 text-white disabled:opacity-50">{opening ? 'Abrindo câmera…' : recording ? 'Parar gravação' : 'Gravar com câmera'}</button>
    ...
    <video ref={video} muted playsInline className={recording || opening ? 'mx-auto max-h-96 rounded-xl bg-black' : 'hidden'} />
